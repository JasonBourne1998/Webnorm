...
import esprima
# JavaScript代码字符串，你可以从文件中读取或直接将其作为字符串传入
javascript_code = """
...
# 读取JavaScript文件
with open('/home/yifannus2023/train-ticket-modify/ts-ui-dashboard/static/assets/js/client_order_list.js', 'r') as file:
    js_content = file.read()

# 使用正则表达式找到methods对象
import re

def extract_methods(js_content):
    pattern = r"methods:\s*\{"
    match = re.search(pattern, js_content)
    if not match:
        return "No 'methods' object found."

    # 开始提取大括号内的内容
    start_index = match.end() - 1  # 包含开始的左大括号
    open_brackets = 1  # 已知起始大括号
    end_index = start_index

    while open_brackets > 0 and end_index < len(js_content):
        end_index += 1
        char = js_content[end_index]
        if char == '{':
            open_brackets += 1
        elif char == '}':
            open_brackets -= 1

    methods_body = js_content[start_index:end_index + 1]
    return methods_body
js_content = """
var component = {
    data: function() {
    ...
    methods: {
        queryMyOrderList() {
            // function body with potentially { nested: { braces: {} } }
        },
        queryForMyOrderThree(path, data) {
            // function body
        }
...
"""
js_string = extract_methods(js_content)
print(js_string)
function_names = re.findall(r'\b(\w+)\s*\(', js_string)
